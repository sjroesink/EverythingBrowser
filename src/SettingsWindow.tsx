...
import { SettingsContent } from "@/components/settings/settings-dialog";
import { useTheme } from "@/hooks/use-theme";
import { getAppDataDir, openPathInExplorer } from "@/services/file-service";
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <SettingsContent
        theme={theme}
        onSetTheme={setTheme}
        onOpenSettingsFile={async () => {
          try {
            const dir = await getAppDataDir();
            await openPathInExplorer(dir);
          } catch (e) {
            console.error("Failed to open settings directory:", e);
          }
        }}
      />
    </div>
